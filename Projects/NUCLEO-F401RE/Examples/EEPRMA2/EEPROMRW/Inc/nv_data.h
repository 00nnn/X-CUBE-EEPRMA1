...
#define EEPROM_SIZE      262144       // 4Mb = 256KB
...
#define DATA_START_ADDR  (HEADER_AREA_SIZE + 8) // Data starts after headers, magic number, and CRC
#define MAX_IMAGE_SIZE   76800         // Example max image size
#define MAGIC_NUMBER     0x02072025  // Unique identifier

extern uint8_t EEPROM[EEPROM_SIZE]; // Simulated EEPROM
...
uint32_t CalculateCRC(const uint8_t* data, uint32_t size);
void InitializeEEPROM();
void LoadImageHeaders();
int FindFreeHeaderSlot();
uint32_t FindFreeDataSpace(uint32_t size);
uint8_t EEPROM_SaveImage(uint16_t id, uint16_t width, uint16_t height, const uint8_t* data);
